...
import { layoutSlots, formFieldsOf, LAYOUT_END } from '@/lib/customFields';

export default function Step1General({ form, updateField, people, uploadingImage, handleImageUpload, customFields = [] }) {
 // Where each custom field was placed. This step renders the whole project
 // form, so every anchor lives here.
 const placedSlots = layoutSlots(formFieldsOf('Project'), customFields);
 const slotFor = (key) => {
  const slot = placedSlots.find((s) => s.key === key);
  if (!slot || slot.custom.length === 0) return null;
  return (
   <div className={key === LAYOUT_END ? 'pt-3 border-t border-border space-y-3' : 'space-y-3'}>
    {key === LAYOUT_END && <p className="text-xs font-semibold text-muted-foreground">שדות נוספים</p>}
 ...
 return (
  <Card className="border-border shadow-sm">
   <CardContent className="p-4 space-y-3">
    ...
    {slotFor('contract_value')}

    {/* Fields generated by an administrator — הגדרות → אדמין → שדות מותאמים.
        Each one is rendered where it was placed; the rest at the foot. */}
    {slotFor(LAYOUT_END)}
   </CardContent>
  </Card>
 );
