...
export default function PopupManagementPanel() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(t("הפופאפ נמחק"));
    },
    onError: () => toast.error(t("המחיקה נכשלה")),
  });
  ...
  const duplicateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['announcementPopups'] });
      toast.success(t("הפופאפ שוכפל"));
    },
    onError: () => toast.error(t("השכפול נכשל")),
  });
  ...
  const archiveMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['announcementPopups'] });
      toast.success(t("הסטטוס עודכן"));
    },
    onError: () => toast.error(t("העדכון נכשל")),
  });
      ...
      <div className="bg-card rounded-lg border border-border shadow-sm overflow-hidden">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 border-b border-border">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-section-title text-foreground">{t("פופאפים והכרזות")}</h2>
          ...
          <div className="flex items-center gap-2">
            ...
            <Button onClick={openNew} className="bg-primary hover:bg-primary/90 text-primary-foreground rounded-full h-9 px-5 text-sm gap-1.5 flex-shrink-0">
              <Plus className="w-4 h-4" /> <span className="hidden sm:inline">{t("פופאפ חדש")}</span>
            </Button>
          </div>
        </div>
          ...
          <EmptyState
            ...
            action={!search ? (
              <Button onClick={openNew} className="bg-primary hover:bg-primary/90 text-primary-foreground rounded-full h-9 px-5 text-sm gap-2">
                <Plus className="w-4 h-4" /> {t("פופאפ חדש")}
              </Button>
            ) : undefined}
          />
        ) : (
            ...
            {filtered.map((popup) => (
              <div key={popup.id} className="flex items-center gap-3 px-4 py-3 hover:bg-muted/30 transition-colors">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    ...
                    {popup.showDismissCheckbox && (
                      <>
                        <span className="text-[11px] text-muted-foreground">·</span>
                        <span className="text-[11px] text-primary">{t("צ'קבוקס \"אל תציג שוב\"")}</span>
                      </>
                    )}
                    <span className="text-[11px] text-muted-foreground">·</span>
                ...
                <div className="flex items-center gap-0.5 flex-shrink-0">
                  <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-foreground" aria-label={t("ערוך פופאפ")} onClick={() => openEdit(popup)}>
                                       <Pencil className="w-3.5 h-3.5" />
                                     </Button>
                                     <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-foreground" aria-label={t("שכפל פופאפ")} onClick={() => duplicateMutation.mutate(popup)}>
                                       <Copy className="w-3.5 h-3.5" />
                                     </Button>
                                     {popup.status === 'archived' ? (
                                       <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-foreground" aria-label={t("הוצא מארכיון")} title={t("הוצא מארכיון")} onClick={() => archiveMutation.mutate({ popup, status: 'draft' })}>
                                         <Archive className="w-3.5 h-3.5" />
                                       </Button>
                                     ) : (
                                       <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-warning" aria-label={t("העבר לארכיון")} title={t("העבר לארכיון")} onClick={() => archiveMutation.mutate({ popup, status: 'archived' })}>
                                         <Archive className="w-3.5 h-3.5" />
                                       </Button>
                                     )}
                                     <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-destructive" aria-label={t("מחק פופאפ")} onClick={() => setDeleteDialog({ open: true, popup })}>
