...
export default function PopupContent({ popup, onCTAClick, onDismissChange }) {
 ...
 return (
  <div dir="rtl"className={config.container}>
   ...
   <div className="p-5">
    <div className="flex items-start gap-3 mb-3">
     ...
     <div className="min-w-0 flex-1">
      <h2 className="text-lg font-bold text-foreground leading-tight text-right">{popup.title}</h2>
     </div>
    </div>

    {popup.content && (
     <div className="text-sm text-foreground leading-relaxed text-right prose-tight"dir="rtl">
      <ReactMarkdown
       components={{
        p: ({ children }) => <p className="mb-2 text-right">{children}</p>,
        li: ({ children }) => <li className="text-right mb-1">{children}</li>,
